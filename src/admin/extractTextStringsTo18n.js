...
async function extractTextStringsToI18n(dir, locales = ["en", "uk"]) {
  ...
  async function processFile(fullPath) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Обробка однорядкового JSX тексту
      const singleLineJsxMatches = line.match(/>([^<{]+)</g) || [];
      for (const match of singleLineJsxMatches) {
  ...
  function filterString(
    ...
    if (line.includes("t(")) {
      if (debug) console.log(`Ignoring t() wrapped text: ${cleanString}`);
      return true;
    }
    ...
    if (
      ...
      if (debug) console.log(`Ignoring log/error: ${cleanString}`);
      return true;
    }
    if (
      inBuilderBlock ||
